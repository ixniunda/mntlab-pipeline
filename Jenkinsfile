...
checkout([$class: 'GitSCM', branches: [[name: BRANCH_NAME]], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: giturl]]])
    ...
    stage('Packaging and Publishing results'){
        ...
        sh "curl -v --user 'admin:admin123' --upload-file $JENKINS_HOME/workspace/EPBYMINW2467/pipeline-job/pipeline-uhramovich-'$BUILD_NUMBER'.tar.gz http://50.50.50.50:8081/repository/maventask-release/nazi/nazi/nazi/pipeline-uhramovich-'$BUILD_NUMBER'.tar.gz"
    }

...
node ("EPBYMINW3088") {
    ...
    stage("Publishing") {
        try {
            ...
            sh "curl -v -u jenkins:jenkins --upload-file ./pipeline-${confBranch}-${BUILD_NUMBER}.tar.gz http://epbyminw3088:8081/repository/maven-custom/com/mnt/pipeline/pipeline-${confBranch}-${BUILD_NUMBER}-${confHN}.tar.gz"
        }
        catch (Exception error) {
